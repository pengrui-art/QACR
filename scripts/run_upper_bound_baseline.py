#!/usr/bin/env python3
"""Phase 1.1 baseline: all tokens go through the deep path (upper bound)."""
...
def main() -> None:
    ...
    deep_route_ratio = float(route_probs[..., 2].mean())
    print("===== Upper Bound Baseline (Task 1.1) =====")
    print(f"model: {model_path}")
    print(f"processor_loaded: {processor.__class__.__name__}")
    print(f"steps: {args.steps}")
    print(f"image_token_shape: {tuple(image_tokens.shape)}")
    print(f"deep_route_ratio: {deep_route_ratio:.6f}")
    print(f"skip_route_ratio: {float(route_probs[..., 0].mean()):.6f}")
    print(f"shallow_route_ratio: {float(route_probs[..., 1].mean()):.6f}")
    print(f"expected_compute_ratio: {deep_route_ratio:.6f}")
    print(f"final_loss: {float(final_loss):.6f}")
    print(f"avg_forward_latency_ms: {avg_latency_ms:.6f}")
    print("status: all tokens deep-path baseline forward/backward succeeded")
