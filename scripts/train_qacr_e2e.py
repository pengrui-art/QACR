...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="QACR end-to-end training")
    # Model
    ...
    p.add_argument("--budget", type=float, default=0.45, help="Compute budget ratio")
    ...
    p.add_argument("--init-checkpoint", type=str, default=None, help="Optional existing QACR checkpoint to initialize router/executor from.")
    ...
    p.add_argument(
        ...
        type=int,
        default=0,
        help="Print timing profile every N optimizer steps (0 means use --log-every).",
    )
    p.add_argument(
        "--profile-sync-cuda",
        action="store_true",
        help="Synchronize CUDA before timing boundaries for more accurate timings.",
    )
    ...
    p.add_argument(
        ...
        type=str,
        default="none",
        choices=["none", "prior_only", "aux_only", "prior_aux"],
        help="Query-conditioned key-token protection mode.",
    )
    p.add_argument("--protection-topk-scale", type=float, default=1.0)
    p.add_argument("--protection-keep-scale", type=float, default=1.0)
    p.add_argument("--protection-deep-scale", type=float, default=1.0)
    p.add_argument("--protection-logit-bias", type=float, default=1.0)
    p.add_argument("--lambda-key-token", type=float, default=0.0)
    return p.parse_args()
...
def _save_checkpoint(
    ...
    torch.save(
        {
            "router": router_to_save.state_dict(),
            ...
            "executor_output_alpha": args.executor_output_alpha,
            "protection_mode": args.protection_mode,
            "protection_topk_scale": args.protection_topk_scale,
            "protection_keep_scale": args.protection_keep_scale,
            "protection_deep_scale": args.protection_deep_scale,
            "protection_logit_bias": args.protection_logit_bias,
            "lambda_key_token": args.lambda_key_token,
        },
        path,
    )
...
def main() -> None:
    ...
    if args.init_checkpoint:
        ...
        router.load_state_dict(init_ckpt["router"])
        executor.load_state_dict(init_ckpt["executor"])
        if main_process:
            log.info("Initialized router/executor from %s", args.init_checkpoint)
    ...
    n_params = sum(p.numel() for p in _unwrap_module(router).parameters()) + sum(
        p.numel() for p in _unwrap_module(executor).parameters()
    )
    if main_process:
    ...
    hook.protection_topk_scale = args.protection_topk_scale
    hook.protection_keep_scale = args.protection_keep_scale
    hook.protection_deep_scale = args.protection_deep_scale
    hook.protection_logit_bias = args.protection_logit_bias
    hook.lambda_key_token = args.lambda_key_token

    # 6. Training
    save_dir = (
        Path(args.save_dir)
        if args.save_dir
        else REPO / "checkpoints" / f"qacr_{args.dataset}_b{args.budget:.2f}"
    )
    if main_process:
    ...
    try:
        for epoch in range(args.epochs):
            ...
            for batch_idx, batch in enumerate(loader):
                ...
                # Pre-compute query embeddings (frozen)
                input_ids = batch["input_ids"]
                embed_device = next(model.get_input_embeddings().parameters()).device
                with torch.no_grad():
                    query_embeds = model.get_input_embeddings()(
                ...
                hook.budget = args.budget
                hook.temperature = temp
                hook.questions = batch.get("_questions")
                hook.question_types = batch.get("_question_types")
                hook.ocr_tokens_batch = batch.get("_ocr_tokens")
                ...
                if (batch_idx + 1) % args.grad_accum == 0 or (batch_idx + 1) == len(
                    ...
                    if global_step % args.log_every == 0 or global_step == 1:
                        ...
                        protected_ratio = _reduce_mean(
                            float(s.protected_ratio), device, distributed, world_size
                        )
                        protected_deep_ratio = _reduce_mean(
                            float(s.protected_deep_ratio), device, distributed, world_size
                        )
                        key_token_loss = _reduce_mean(
                            (
                                float(s.protection_loss.detach())
                        ...
                        entry = {
                            ...
                            "protected_ratio": round(protected_ratio, 4),
                            "protected_deep_ratio": round(protected_deep_ratio, 4),
                            "key_token_loss": round(key_token_loss, 6),
